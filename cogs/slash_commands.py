...
class SlashCommands(commands.Cog):
    ...
    @nextcord.slash_command(name="set_color", description="Set the embed color")
    async def set_color(self, interaction: Interaction, color: str):
        try:
            if len(color) != 6 or not all(c in "0123456789ABCDEFabcdef" for c in color):
                await interaction.response.send_message("Invalid color format! Please provide a hex color code (e.g., FF5733).")
                return
    ...
    @nextcord.slash_command(name="print_stats", description="Prints server and command statistics to terminal")
    async def print_stats(self, interaction: Interaction):
        try:
            ...
            await interaction.response.send_message("Statistics printed to terminal.")
        except Exception as e:
            await interaction.response.send_message(f"Error: {e}")
            print(f"Error: {e}")
