    ...
    async def show_settings_button(self, button: Button, interaction: Interaction):
        ...
        try:
            ...
            for feed_dto in feed_bll.get_all_feed():
                ...
                channel_dto = channel_bll.get_channel_by_channel_id(
                    str(channel_id))
                ...
                if channel:  # Kiểm tra xem kênh có tồn tại không
                    for server in self.bot.guilds:
                        if channel in server.channels:
                            ...
                            channel_info = f"- **{channel_dto.channel_name}** (`{channel_id}`) - [{feed_dto.title_feed}]({feed_dto.link_feed})"  # type: ignore
            ...
            for server_name, channels in server_data.items():
                embed.add_field(
                    name=server_name,
                    value="\n".join(
                        channels) if channels else "No channels found.",
                    inline=False
                )
    ...
    async def show_servers_button(self, button: Button, interaction: Interaction):
        ...
        try:
            ...
            embed = EmbedCustom(
                id_server=id_server,
                title="Servers",
                description=f"The bot joined {
                    num} guilds: **{', '.join(guild_names)}**",
                color=self.color
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
