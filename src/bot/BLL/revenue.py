...
class Revenue:
    ...
    # type:ignore
    def get_total_revenue_by_year(self, year: int, premium_id: str | None = None) -> float:
        ...
        for transaction in transaction_history_list:
            ...
            if transaction_date.year == year:
                if premium_id:
                    matching_qr_paycode = next(
                        (qr for qr in qr_paycode_list if qr.qr_code == transaction.qr_code), None)
    ...
    # type:ignore
    def get_total_revenue_between_dates(self, start_date: datetime, end_date: datetime, premium_id: str | None = None) -> float:
        qr_paycode_list = self.__qr_paycode_bll.get_all_qr_pay_code()
        transaction_history_list = self.__transaction_history_bll.get_all_transaction_history()
        total = 0.0
    ...
    # type:ignore
    def get_total_revenue_by_quarter(self, year: int, quarter: int, premium_id: str | None = None) -> float:
        qr_paycode_list = self.__qr_paycode_bll.get_all_qr_pay_code()
        transaction_history_list = self.__transaction_history_bll.get_all_transaction_history()
        total = 0.0
