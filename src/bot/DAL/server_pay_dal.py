...
class ServerPayDAL(BaseDAL):
    ...
    def update_server_pay(self, server_pay: ServerPayDTO):
        self.open_connection()
        self.cursor.execute("UPDATE server_pay SET is_pay=? WHERE server_id=?;",
                            (server_pay.server_dto, server_pay.is_pay))
        self.connection.commit()
        self.close_connection()
        return True
    ...
    def get_server_pay_by_server_id(self, server_id: str):
        self.open_connection()
        self.cursor.execute(
            "SELECT server_id, is_pay FROM server_pay Where server_id=?", (server_id,))
    ...
    def delete_server_pay_by_server_id(self, server_id: str):
        self.open_connection()
        self.cursor.execute(
            "DELETE FROM server_pay WHERE server_id=?;", (server_id,))
        self.connection.commit()
        self.close_connection()
        return True
