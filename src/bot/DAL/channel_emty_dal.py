...
class ChannelEmtyDAL(BaseDAL):
        ...
        try:
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS tbl_channel_emty(
                    ...
                    FOREIGN KEY (link_feed) REFERENCES tbl_emty(link_feed),
                    FOREIGN KEY (link_atom_feed) REFERENCES tbl_emty(link_atom_feed)
                )
            ''')
    ...
    def insert_channel_emty(self, channel_emty_dto: ChannelEmtyDTO):
        ...
        try:
            with self.connection:
                self.cursor.execute('''
                INSERT INTO tbl_channel_emty (channel_id, link_emty, link_feed, link_atom_feed)
                VALUES (?, ?, ?, ?)
                ''', (channel_emty_dto.get_channel().get_channel_id(), 
                      channel_emty_dto.get_emty().get_link_emty(),
                      channel_emty_dto.get_emty().get_link_feed(), 
                      channel_emty_dto.get_emty().get_link_atom_feed()))
                self.connection.commit()
                return True
    ...
    def get_all_channel_emty(self) -> List[ChannelEmtyDTO]:
        ...
        try:
            ...
            SELECT c.channel_id, c.server_id, c.channel_name, c.is_active,
                f.link_emty, f.link_feed, f.link_atom_feed, f.title_emty, f.description_emty, f.image_emty, f.pubdate_emty 
            ...
            return [ChannelEmtyDTO(ChannelDTO(row[0], row[1], row[2], bool(row[3])),
                                    EmtyDTO(row[4], row[5], row[6], row[7], row[8], row[9], row[10])) for row in rows]
        except sqlite3.Error as e:
    ...
    def get_all_channel_emty_by_channel_id_and_link_feed(self, channel_id: str, link_feed: str) -> List[ChannelEmtyDTO]:
        self.open_connection()
        try:
            self.cursor.execute('''
            SELECT c.channel_id, c.server_id, c.channel_name, c.is_active,
            ...
            JOIN tbl_channel c ON cf.channel_id = c.channel_id
            JOIN tbl_emty f ON cf.link_emty = f.link_emty
            WHERE cf.channel_id = ?, cf.link_feed = ?
            ''', (channel_id, link_feed))
            rows = self.cursor.fetchall()
            return [ChannelEmtyDTO(ChannelDTO(row[0], row[1], row[2], bool(row[3])),
                                    EmtyDTO(row[4], row[5], row[6], row[7], row[8], row[9], row[10])) for row in rows]
    ...
    def get_all_channel_emty_by_channel_id_and_link_atom_feed(self, channel_id: str, link_atom_feed: str) -> List[ChannelEmtyDTO]:
        self.open_connection()
        try:
            self.cursor.execute('''
            SELECT c.channel_id, c.server_id, c.channel_name, c.is_active,
    ...
    def get_channel_emty_by_channel_id_and_link_emty(self, channel_id: str, link_emty: str) -> Optional[ChannelEmtyDTO]:
        try:
            ...
            return ChannelEmtyDTO(ChannelDTO(row[0], row[1], row[2], bool(row[3])),
                                    EmtyDTO(row[4], row[5], row[6], row[7], row[8], row[9], row[10])) 
        except sqlite3.Error as e:
            logger.error(f"Error fetching all data from `tbl_channel_emty`: {e}")
            return None
        finally:
