...
class UserChannelDAL(BaseDAL):
        ...
        try:
            with self.connection:
                self.cursor.execute('''
                    ...
                    VALUES (?, ?, ?)
                    ''', (userchannel_dto.get_user().get_user_id(), userchannel_dto.get_channel().get_channel_id(), userchannel_dto.get_date()))
                self.connection.commit()
                logger.info(f"Data inserted into 'tbl_user_channel' successfully.")
                return True
        except sqlite3.Error as e:
    ...
    def get_all_user_channel(self) -> List[UserChannelDTO]:
        ...
        try:
            ...
            logger.error(f"Error fetching all data from 'tbl_user_channel': {e}")
            return []
        finally:
            self.close_connection()
    ...
    def delete_user_channel_by_user_id_and_channel_id(self, user_id: str, channel_id: str) -> bool:
        ...
        try:
            with self.connection:
                ...
                ''', (user_id, channel_id))
                self.connection.commit()
                logger.info(f"Data deleted from 'tbl_user_channel' successfully.")
                return True
        except sqlite3.Error as e:
