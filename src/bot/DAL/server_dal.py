...
class ServerDAL(BaseDAL):
        ...
        try:
            with self.connection:
                self.cursor.execute('''
                    ...
                    VALUES (?, ?, ?)
                    ''', (server_dto.server_id, server_dto.server_name, server_dto.hex_color))
                self.connection.commit()
                logger.info(f"Data inserted successfully into 'tbl_server'.")
                return True
        except sqlite3.IntegrityError as e:
            logger.error(f"Server with server_id={
                         server_dto.server_id} already exists in 'tbl_server'")
            return False
        except sqlite3.Error as e:
    ...
    def delete_all_server(self) -> bool:
        ...
        try:
            with self.connection:
                ...
                self.connection.commit()
                logger.info(
                    f"All data deleted successfully from 'tbl_server'.")
                return True
        except sqlite3.Error as e:
    ...
    def update_server(self, server_dto: ServerDTO) -> bool:
        ...
        try:
            with self.connection:
                ...
                WHERE server_id = ?
                ''', (server_dto.server_name, server_dto.hex_color, server_dto.is_active, server_dto.server_id))
                self.connection.commit()
                logger.info(f"ALl data updated successfully in 'tbl_server'.")
                return True
        except sqlite3.Error as e:
            logger.error(
                f"Error updating data by server_id in 'tbl_server': {e}")
            return False
        finally:
            self.close_connection()
    ...
    def get_server_by_server_id(self, server_id: str) -> Optional[ServerDTO]:
        ...
        try:
            ...
            if row:
                return ServerDTO(row[0], row[1], row[2], row[3])
            else:
                return None
        except sqlite3.Error as e:
            logger.error(
                f"Error fetching data by server_id from 'tbl_server': {e}")
            return None
        finally:
            self.close_connection()
    ...
    def get_all_server(self, ignore_state=False, is_active=True) -> List[ServerDTO]:
        ...
        try:
            ...
            if rows:
                return [ServerDTO(row[0], row[1], row[2], row[3]) for row in rows]
            else:
                return []
        except sqlite3.Error as e:
