...
class FeedEmtyDAL:
    ...
    def create_table(self):
        try:
            ...
            CREATE TABLE IF NOT EXISTS tbl_feed_emty(
                linkAtom_feed TEXT,
                link_emty TEXT,
                PRIMARY KEY (linkAtom_feed, link_emty),
    ...
    def insertFeedEmty(self, feed_emty_dto: FeedEmtyDTO) -> bool:
        try:
            with self.__connection:
                self.__cursor.execute('''
                    INSERT INTO tbl_feed_emty (linkAtom_feed, link_emty)
                    VALUES (?, ?)
                ''', (feed_emty_dto.getFeed().getLinkAtom_feed(), feed_emty_dto.getEmty().getLink_emty()))
                self.__connection.commit()
    ...
    def deleteFeedEmtyByLinkAtom_feedAndLink_emty(self, linkAtom_feed: str, link_emty: str) -> bool:
        try:
            with self.__connection:
                self.__cursor.execute('''
                DELETE FROM tbl_feed_emty WHERE linkAtom_feed = ? AND link_emty = ?
                ''', (linkAtom_feed, link_emty))
                self.__connection.commit()
    ...
    def updateFeedEmtyByLinkAtom_feedAndLink_emty(self, linkAtom_feed: str, link_emty: str, feed_emty_dto: FeedEmtyDTO) -> bool:
        try:
            with self.__connection:
                self.__cursor.execute('''
                UPDATE tbl_feed_emty SET link_emty = ?, linkAtom_feed = ?
                WHERE link_emty = ? AND linkAtom_feed = ?
                ''', (feed_emty_dto.getEmty().getLink_emty(), feed_emty_dto.getFeed().getLinkAtom_feed(), link_emty, linkAtom_feed))
                self.__connection.commit()
    ...
    def getFeedEmtyByLinkAtom_feedAndLink_emty(self, linkAtom_feed: str, link_emty: str) -> Optional[FeedEmtyDTO]:
        try:
            with self.__connection:
                ...
                JOIN tbl_feed f ON fe.linkAtom_feed = f.linkAtom_feed
                JOIN tbl_emty e ON fe.link_emty = e.link_emty
                WHERE fe.linkAtom_feed = ? AND fe.link_emty = ?
                ''', (linkAtom_feed, link_emty))
            ...
            print(f"Error fetching data from 'tbl_feed_emty' by linkAtom_feed and link_emty: {e}")
            return None
    ...
    def getFeedEmtyByLinkAtom_feed(self, linkAtom_feed: str) -> Optional[FeedEmtyDTO]:
        try:
            ...
            print(f"Error fetching data from 'tbl_feed_emty' by linkAtom_feed: {e}")
            return None
