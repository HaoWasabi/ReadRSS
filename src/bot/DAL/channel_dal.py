...
class ChannelDAL(BaseDAL):
        ...
        try:
            with self.connection:
                self.cursor.execute('''
                    ...
                    VALUES (?, ?, ?)
                    ''', (channel_dto.channel_id, channel_dto.server_id, channel_dto.channel_name))
                self.connection.commit()
                logger.info(f"Data inserted into 'tbl_channel' successfully.")
                return True
        except sqlite3.IntegrityError as e:
            logger.error(f"Channel with channel_id={
                         channel_dto.channel_id} already exists in 'tbl_channel'")
            return False
        except sqlite3.Error as e:
    ...
    def delete_all_channel(self) -> bool:
        ...
        try:
            with self.connection:
                ...
                self.connection.commit()
                logger.info(
                    f"All data deleted from 'tbl_channel' successfully.")
                return True
        except sqlite3.Error as e:
    ...
    def update_channel(self, channel_dto: ChannelDTO) -> bool:
        ...
        try:
            with self.connection:
                ...
                WHERE channel_id = ?
                ''', (channel_dto.channel_name, channel_dto.is_active, channel_dto.channel_id))
                self.connection.commit()
    ...
    def get_channel_by_channel_id(self, channel_id: str) -> Optional[ChannelDTO]:
        ...
        try:
            ...
            logger.error(
                f"Error fetching data from 'tbl_channel' by link_channel: {e}")
            return None
        finally:
            self.close_connection()
    ...
    def get_all_channel(self, ignore_state=False, is_active=True) -> List[ChannelDTO]:
        ...
        try:
            ...
            if rows:
                return [ChannelDTO(*row) for row in rows]
            else:
                return []
        except sqlite3.Error as e:
