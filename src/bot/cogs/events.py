...
from ..DTO import ChannelDTO, ServerDTO, ColorDTO
from ..BLL import ChannelBLL, ServerBLL, FeedBLL, EmtyBLL
from ..GUI import EmbedFeed, EmbedCustom
...
class Events(CommandsCog):
            ...
            for guild in guilds:
                ...
                if server_dto not in list_server:
                    ...
                    matching_server = next(
                        (s for s in list_server if s.server_id == server_dto.server_id), None)
                    if matching_server and matching_server.server_name != server_dto.server_name:
                ...
                for channel in guild.channels:
                    ...
                    channel_dto = ChannelDTO(
                        id_channel, name_channel, id_server)

                    matching_channel = next(
                        (ch for ch in list_channel if ch.channel_id == channel_dto.channel_id), None)
                    if matching_channel and matching_channel.channel_name != channel_dto.channel_name:
    ...
    async def on_ready(self):
        ...
        logger.info("Current commands: %s", str(
            [command.name for command in self.bot.commands]))
        logger.info("Current slash commands: %s", str(
            [command.name for command in self.bot.get_application_commands()]))
                ...
                async for message1 in message.channel.history(limit=20):
                    if message1.author.id != self.bot.user.id:
                        ...
                        if (prompt1.startswith(f'<@{self.bot.user.id.__str__()}> ')):
                            prompt1 = prompt1[len(
                                f'<@{self.bot.user.id.__str__()}> '):]

                        if len(prompt1) == 0:
                            continue

                        history.append({"role": "user", "parts": f'{
                                       message1.author.name}: "{prompt1}"'})
                    else:
                        history.append(
                            {"role": "model", "parts": message1.content})
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            command_list_2 = ", ".join(
                available_slash_commands)  # type: ignore
...
- The current slash commands have: {command_list_2}
            ...
            embed.set_thumbnail(
                url="https://cdn-longterm.mee6.xyz/plugins/welcome/images/911798642518663208/d7a41040adf3036620000c397fbfa21a487c9e5bb1db698fd3081ed541f4b5c1.gif")
