...
class ChannelFeedDAL:
        ...
        try:
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS tbl_channel_feed(
                    link_feed TEXT,
                    id_channel TEXT,
                    PRIMARY KEY (id_channel, link_feed)
                )
            ''')
    ...
    def insertChannelFeed(self, channel_feed_dto: ChannelFeedDTO) -> bool:
        try:
            with self.connection:
                self.cursor.execute('''
                INSERT INTO tbl_channel_feed (id_channel, link_feed)
                VALUES (?, ?)
                ''', (channel_feed_dto.getFeed().getLink_feed(), channel_feed_dto.getChannel().getId_channel()))
                self.connection.commit()
                return True
        except sqlite3.Error as e:
    ...
    def deleteChannelFeedById_channelAndLink_feed(self, id_channel: str, link_feed: str) -> bool:
        try:
            with self.connection:
                self.cursor.execute('''
                DELETE FROM tbl_channel_feed WHERE link_feed = ? AND id_channel = ?
    ...
    def updateChannelFeedById_channelAndLink_feed(self, id_channel: str, link_feed: str, channel_feed_dto: ChannelFeedDTO) -> bool:
        try:
            with self.connection:
                self.cursor.execute('''
                UPDATE tbl_channel_feed SET link_feed = ?, id_channel = ?
                WHERE link_feed = ? AND id_channel= ?
                ''', (channel_feed_dto.getFeed().getLink_feed(), channel_feed_dto.getChannel().getId_channel(), link_feed, id_channel))
                self.connection.commit()
                return True
        except sqlite3.Error as e:
    ...
    def getChannelFeedById_channelAndLink_feed(self, id_channel: str, link_feed: str) -> Optional[ChannelFeedDTO]:
        try:
            ...
            JOIN tbl_feed f ON cf.link_feed = f.link_feed
            WHERE cf.link_feed = ? AND cf.id_channel = ?
            ''', (link_feed, id_channel))
